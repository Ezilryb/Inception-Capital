 ...
 * ============================================================
 *  PATCHED v3: expose canvas as TextureAtlas.canvas for UIManager
 * ============================================================
 */
'use strict';
...
function drawTile(ctx, tileX, tileY, def) {
  ...
  switch (def.pat) {
    case 'solid': fillBase(); break;
    case 'noise':
      fillBase();
      for (let py=0;py<TILE_SIZE;py++) for (let px=0;px<TILE_SIZE;px++) {
        if (rng()<0.3){ const v=(rng()*30-15)|0; ctx.fillStyle=`rgb(${r+v},${g+v},${b+v})`; ctx.fillRect(x0+px,y0+py,1,1); }
      }
      break;
    case 'planks':{
      fillBase(); ctx.fillStyle=`rgb(${r2},${g2},${b2})`;
      for(let py=0;py<TILE_SIZE;py+=4){ctx.fillRect(x0,y0+py,TILE_SIZE,1); const o=(py/4%2===0)?8:0; ctx.fillRect(x0+o,y0+py,1,4);}
      break;}
    case 'bricks':{
      fillBase(); ctx.fillStyle=`rgb(${r2},${g2},${b2})`;
      for(let py=0;py<TILE_SIZE;py+=4){ctx.fillRect(x0,y0+py,TILE_SIZE,1); const off=(py/4%2===0)?8:4; for(let px=off;px<TILE_SIZE;px+=8)ctx.fillRect(x0+px,y0+py,1,4);}
      break;}
    case 'cobble':{
      ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE);
      for(let i=0;i<12;i++){const cx2=(rng()*TILE_SIZE)|0,cy=(rng()*TILE_SIZE)|0,w=2+(rng()*5)|0,h=2+(rng()*5)|0,v=(rng()*40-20)|0; ctx.fillStyle=`rgb(${r+v},${g+v},${b+v})`; ctx.fillRect(x0+cx2,y0+cy,w,h);}
      ctx.fillStyle=`rgb(${r2},${g2},${b2})`; for(let i=0;i<20;i++){const cx2=(rng()*TILE_SIZE)|0,cy=(rng()*TILE_SIZE)|0; ctx.fillRect(x0+cx2,y0+cy,1+(rng()*3|0),1);}
      break;}
    case 'ore':{
      ctx.fillStyle='rgb(120,120,120)'; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE);
      for(let py=0;py<TILE_SIZE;py++) for(let px=0;px<TILE_SIZE;px++) if(rng()<0.12){const v=(rng()*20-10)|0; ctx.fillStyle=`rgb(${110+v},${110+v},${110+v})`; ctx.fillRect(x0+px,y0+py,1,1);}
      for(let i=0;i<6;i++){const cx2=2+(rng()*(TILE_SIZE-4))|0,cy=2+(rng()*(TILE_SIZE-4))|0,v=(rng()*20-10)|0; ctx.fillStyle=`rgb(${r2+v},${g2+v},${b2+v})`; ctx.beginPath(); ctx.arc(x0+cx2,y0+cy,1.5+rng()*1.5,0,Math.PI*2); ctx.fill();}
      break;}
    case 'log':{ fillBase(); for(let py=0;py<TILE_SIZE;py++){const v=(Math.sin(py/2)*8)|0; ctx.fillStyle=`rgb(${r+v},${g+v},${b+v})`; ctx.fillRect(x0,y0+py,TILE_SIZE,1);} ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0+6,y0+6,4,4); break;}
    case 'rings':{ fillBase(); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.beginPath(); ctx.arc(x0+8,y0+8,5,0,Math.PI*2); ctx.arc(x0+8,y0+8,2,0,Math.PI*2); ctx.stroke(); break;}
    case 'leaves':{
      for(let py=0;py<TILE_SIZE;py++) for(let px=0;px<TILE_SIZE;px++){const v=(rng()*40-20)|0,on=rng()>0.1; if(on){ctx.fillStyle=`rgb(${r+v},${g+v},${b+v})`; ctx.fillRect(x0+px,y0+py,1,1);}else ctx.clearRect(x0+px,y0+py,1,1);}
      break;}
    case 'glass':{ ctx.clearRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgba(${r},${g},${b},0.3)`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0,y0,TILE_SIZE,1);ctx.fillRect(x0,y0+TILE_SIZE-1,TILE_SIZE,1);ctx.fillRect(x0,y0,1,TILE_SIZE);ctx.fillRect(x0+TILE_SIZE-1,y0,1,TILE_SIZE); break;}
    case 'water':{ ctx.fillStyle=`rgba(${r},${g},${b},0.8)`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgba(${r2},${g2},${b2},0.4)`; for(let py=0;py<TILE_SIZE;py+=3)ctx.fillRect(x0,y0+py,TILE_SIZE,1); break;}
    case 'lava':{ fillBase(); for(let i=0;i<10;i++){const cx2=(rng()*TILE_SIZE)|0,cy=(rng()*TILE_SIZE)|0; ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0+cx2,y0+cy,2+(rng()*4|0),2+(rng()*4|0));} break;}
    case 'cross':{ ctx.clearRect(x0,y0,TILE_SIZE,TILE_SIZE); const cx2=x0+TILE_SIZE/2,cy2=y0+TILE_SIZE/2; for(let i=-5;i<=5;i++){ctx.fillStyle=`rgb(${r+(rng()*20-10|0)},${g+(rng()*20-10|0)},${b+(rng()*10-5|0)})`; ctx.fillRect(cx2-1+i*0.3,cy2-6+i,2,2); ctx.fillRect(cx2-1-i*0.3,cy2-6+i,2,2);} break;}
    case 'metal':{ fillBase(); for(let py=0;py<TILE_SIZE;py+=4){const v=(rng()*20-10)|0; ctx.fillStyle=`rgb(${r+v},${g+v},${b+v})`; ctx.fillRect(x0,y0+py,TILE_SIZE,2);} break;}
    case 'grass_side':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0+3,TILE_SIZE,TILE_SIZE-3); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0,y0,TILE_SIZE,3); for(let px=0;px<TILE_SIZE;px++){const v=(rng()*10)|0; ctx.fillStyle=`rgb(${r2-v},${g2-v},${b2-v})`; ctx.fillRect(x0+px,y0+2+v%2,1,1+(v%2));} break;}
    case 'sbrick':{ fillBase(); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; for(let py=0;py<TILE_SIZE;py+=8){ctx.fillRect(x0,y0+py,TILE_SIZE,1); for(let px=0;px<TILE_SIZE;px+=8)ctx.fillRect(x0+px,y0+py,1,8);} break;}
    case 'glowstone':{ for(let py=0;py<TILE_SIZE;py++) for(let px=0;px<TILE_SIZE;px++){const v=(rng()*40-20)|0; ctx.fillStyle=`rgb(${r+v},${g+v},${b+v})`; ctx.fillRect(x0+px,y0+py,1,1);} for(let i=0;i<5;i++){const cx2=(2+rng()*(TILE_SIZE-4))|0,cy=(2+rng()*(TILE_SIZE-4))|0; ctx.fillStyle='rgb(255,240,150)'; ctx.fillRect(x0+cx2,y0+cy,2,2);} break;}
    case 'magic':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); for(let i=0;i<8;i++){const px2=(rng()*TILE_SIZE)|0,py2=(rng()*TILE_SIZE)|0; ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0+px2,y0+py2,1+(rng()*2|0),1+(rng()*2|0));} break;}
    case 'ice':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle='rgba(255,255,255,0.5)'; for(let i=0;i<3;i++){const ax=(rng()*TILE_SIZE)|0,ay=(rng()*TILE_SIZE)|0; ctx.fillRect(x0+ax,y0+ay,rng()*8|0,1);} break;}
    case 'shroom':{ ctx.clearRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; for(let i=0;i<5;i++){const cx2=2+(rng()*10)|0,cy=2+(rng()*10)|0; ctx.fillRect(x0+cx2,y0+cy,2,2);} break;}
    case 'torch':{ ctx.clearRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0+7,y0+0,2,4); ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0+6,y0+4,4,10); break;}
    case 'chest':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0,y0+7,TILE_SIZE,2); ctx.fillRect(x0+6,y0+4,4,5); break;}
    case 'craft':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; for(let i=0;i<3;i++) for(let j=0;j<3;j++) ctx.fillRect(x0+2+i*5,y0+2+j*5,4,4); break;}
    case 'furnace':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0+4,y0+4,8,8); break;}
    case 'sandstone':{ fillBase(); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0,y0+5,TILE_SIZE,1); ctx.fillRect(x0,y0+10,TILE_SIZE,1); break;}
    case 'melon':{ fillBase(); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; for(let px=0;px<TILE_SIZE;px+=3)ctx.fillRect(x0+px,y0,1,TILE_SIZE); break;}
    case 'cactus':{ fillBase(); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; for(let py=0;py<TILE_SIZE;py+=3)ctx.fillRect(x0,y0+py,TILE_SIZE,1); break;}
    case 'shelf':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0,TILE_SIZE,TILE_SIZE); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; for(let i=0;i<3;i++)ctx.fillRect(x0+i*5+1,y0+2,3,12); ctx.fillStyle='rgb(200,160,100)'; for(let i=0;i<3;i++)ctx.fillRect(x0+i*5+1,y0+4,3,3); break;}
    case 'snow_side':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0+3,TILE_SIZE,TILE_SIZE-3); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0,y0,TILE_SIZE,3); break;}
    case 'dirt_top':{ ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x0,y0+3,TILE_SIZE,TILE_SIZE-3); ctx.fillStyle=`rgb(${r2},${g2},${b2})`; ctx.fillRect(x0,y0,TILE_SIZE,3); break;}
    default: fillBase(); break;
  }
}

export class TextureAtlas {
  /** Exposed for UIManager — set after createProceduralAtlas() is called */
  static canvas = null;
  ...
  static createProceduralAtlas() {
    ...
    // ── Save canvas reference for UIManager item icons ───────
